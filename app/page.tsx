          ...
          <div className="flex gap-2 py-3 no-scroll overflow-x-auto">
            {Object.entries(BLOCK_LABELS).map(([key, label]) => (
              <Link key={key} href={`/program?block=${key}`}>
                <span className="flex-shrink-0 flex items-center gap-1.5 rounded-full transition-all active:scale-95"
                  style={{
                  ...
                  onTouchStart={e => {
                    ...
                    el.style.boxShadow = `0 0 10px ${BLOCK_DOT[key]}55, 0 0 0 1px ${BLOCK_DOT[key]}60`
                    el.style.background = `${BLOCK_DOT[key]}22`
                  }}
                  onTouchEnd={e => {
                    const el = e.currentTarget as HTMLElement
                    el.style.boxShadow = 'none'
                    el.style.background = BLOCK_ACCENT[key]
                  }}
                >
